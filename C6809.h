...
public:
	...
	Byte fetch_byte() { Byte data = bus->read(PC); PC++; return data; }
	Word fetch_word() { Word data = bus->read_word(PC); PC += 2; return data; }

	Byte read(Word offset)						{ return bus->read(offset); }
	void write(Word offset, Byte data)			{ bus->write(offset, data); }
	Word read_word(Word offset)					{ return bus->read_word(offset); }
	void write_word(Word offset, Word data)		{ bus->write_word(offset, data); }

	Byte debug_read(Word offset)				{ return bus->debug_read(offset);}
	void debug_write(Word offset, Byte data)	{ bus->debug_write(offset, data); }
	Word debug_read_word(Word offset)			{ return bus->debug_read_word(offset); }
	void debug_write_word(Word offset, Word data) { bus->debug_write_word(offset, data); }
