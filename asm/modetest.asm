...
1		cmpx	#VIDEO_END	; at the end of the buffer?
...
2		ldx	#VIDEO_START	; start beginning of video buffer
4		cmpx	#VIDEO_END		; until the end of the video buffer
		bge	3f				; restart when past the end
		inc	,x+				; increment character
		inc	,x+				; increment the attribute
		bra	4b				; loop until done
3		; SCROLL THE EXTENDED SCREEN BUFFER
...
6		ldb	var_mode_index
		...
		sta	GFX_FLAGS
		bra	continue
5		clr	var_mode_index
		bra	6b
continue	; WAS [ESCAPE] PRESSED
		lda	CHAR_Q_LEN
		beq	2b
		; check for ESCAPE
		lda	CHAR_POP
